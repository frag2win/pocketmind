...
object GemmaPromptFormatter {
    ...
    /**
     * Converts Room-backed chat history rows into a List<Message> for ConversationConfig.initialMessages.
     * Preserves PDF-context anchoring and character-budget truncation.
     */
    fun buildInitialMessages(
        ...
        val lastPdfIndex = history.indexOfLast { 
        ...
        val relevantMessages = if (lastPdfIndex != -1) {
            history.subList(lastPdfIndex, history.size)
        } else {
            history
        }

        // Iterate backwards and stop when exceeding character limit
        ...
        return finalMessages.map { message ->
            ...
            if (message.role == "user") {
                Message.user(cleanContent)
            } else {
                Message.model(cleanContent)
            }
        }
    }
    ...
    fun formatSummaryPrompt(userMessage: String, assistantResponse: String): String {
        ...
        return "Summarize the following exchange into a 3-5 word title. " +
